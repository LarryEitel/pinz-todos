...
function* filterIter<T>(iter: Iterable<T>, p: (t: T, index: number) => boolean): Iterable<T> {
	let index = 0;
	for (const t of iter) {
		if (p(t, index++)) {
			yield t;
...
function* mapIter<T, U>(iter: Iterable<T>, f: (t: T, index: number) => U): Iterable<U> {
	let index = 0;
	for (const t of iter) {
		yield f(t, index++);
	}
...
export function* mergeIterables<T>(
	...
	try {
		...
		while (current.some((c) => !c.done)) {
			const min = current.reduce((acc: [T, number] | undefined, c, i): [T, number] | undefined => {
				if (c.done) {
					return acc;
				}
			...
			if (lastYielded !== undefined && distinct && comparator(lastYielded, min[0]) === 0) {
				continue;
			}
			lastYielded = min[0];
			yield min[0];
