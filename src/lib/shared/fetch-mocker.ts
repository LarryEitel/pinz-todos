...
type FetchSpy = vitest.MockInstance<
	(...args: [input: string | Request | URL, init?: RequestInit | undefined]) => Promise<Response>
>;
...
export class FetchMocker {
	...
	default(errorCodeOrResult: number | Record<string, unknown>, message?: string): this {
	...
	error(method: Method, urlSubstring: string, code: number, message?: string): this {
		this.handlers.push({
			method,
			urlSubstring,
	...
	requests(): [method: string, url: string][] {
		return this.spy.mock.calls.map(([input, init]) => [init?.method ?? 'GET', input.toString()]);
	}
	...
	jsonPayloads(): unknown[] {
		return this.spy.mock.calls.map(([_, init]) => JSON.parse(String(init?.body)));
	}
}
