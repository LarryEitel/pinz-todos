...
import { mergeIterables, wrapIterable } from './iterables';
...
describe('mergeIterables', () => {
	...
	test('fuzz', () => {
		fc.assert(
			fc.property(fc.array(fc.array(fc.integer())), fc.boolean(), (arrays, noDupes) => {
				...
				expect([...result]).toEqual(noDupes ? [...new Set(expected)] : expected);
			})
		);
	});
