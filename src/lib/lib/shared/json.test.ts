...
import { type JSONValue, assertJSONValue, deepEqual, isJSONValue } from './json';

test('JSON deep equal', () => {
	const t = (a: JSONValue | undefined, b: JSONValue | undefined, expected = true) => {
		const res = deepEqual(a, b);
		const res2 = deepEqual(b, a);
		expect(res).toBe(res2);
		if (res !== expected) {
			throw new Error(JSON.stringify(a) + (expected ? ' === ' : ' !== ') + JSON.stringify(b));
		}
	};
...
test('assertJSONValue', () => {
	...
	assertJSONValue(Object.create({ b: Symbol() }, Object.getOwnPropertyDescriptors({ a: 1 })));
