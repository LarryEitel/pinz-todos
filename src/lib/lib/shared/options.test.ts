...
test.each([
	...
	[
		'--bool=true flag',
		['--litestream=true', '--replica-db-file=/tmp/replica.db', '--log-level=info'],
		{},
		false,
		{
			port: 4848,
	...
	[
		'--bool 1 flag',
		['--litestream', '1', '--replica-db-file=/tmp/replica.db', '--log-level=info'],
		{},
		false,
		{
			port: 4848,
	...
	[
		'--bool False flag',
		['--litestream', 'False', '--replica-db-file=/tmp/replica.db', '--log-level=info'],
		{},
		false,
		{
			port: 4848,
	...
	[
		'unknown flags',
		['--foo', '--replica-db-file', '/tmp/replica.db', 'bar', '--log-level=info', '--baz=3'],
		{},
		false,
		{
			port: 4848,
	...
	const parsed = parseOptionsAdvanced(options, argv, 'Z_', true, allowPartial, env);
	...
	const reparsed = parseOptionsAdvanced(options, [], 'Z_', true, allowPartial, parsed.env);
...
test.each([
	['missing required flag', { requiredFlag: v.string() }, [], 'Missing property requiredFlag'],
	[
	...
	[
		...
		// Options type forbids this, but cast to verify runtime check.
		{
			bad: v.tuple([v.union(v.literal('a'), v.literal('b')), v.union(v.literal(1), v.literal(2))])
		} as Options,
		[],
		'--bad has mixed types string,number'
	],
	['bad number', { num: v.number() }, ['--num=foobar'], 'Invalid input for --num: "foobar"'],
	['bad bool', { bool: v.boolean() }, ['--bool=yo'], 'Invalid input for --bool: "yo"']
...
test('--help', () => {
	...
	expect(() => parseOptions(options, ['--help'], 'Z_', {}, logger, exit)).toThrow(ExitAfterUsage);
...
test('-h', () => {
	const logger = { info: vi.fn() };
	expect(() => parseOptions(options, ['-h'], 'ZERO_', {}, logger, exit)).toThrow(ExitAfterUsage);
	expect(logger.info).toHaveBeenCalled();
	expect(stripAnsi(logger.info.mock.calls[0][0])).toMatchInlineSnapshot(`
    "
     --port, -p number                  default: 4848                                                        
...
test('unknown arguments', () => {
	...
	expect(() => parseOptions(options, ['--shardID', 'foo'], '', {}, logger, exit)).toThrow(
		ExitAfterUsage
	);
