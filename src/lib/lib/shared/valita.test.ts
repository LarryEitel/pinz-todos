...
test('basic', () => {
	...
	t(v.tuple([v.number()]), [], 'Expected array with length 1. Got array with length 0');
...
test('union error message', () => {
	...
	for (const s of [type, v.union(type), v.union(v.union(type))]) {
		...
		t(s, ['b', { b: 'not a number' }], 'Expected number at 1.b. Got "not a number"');
