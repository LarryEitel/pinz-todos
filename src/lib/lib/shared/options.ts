...
type Required = RequiredOptionType | (WrappedOptionType & { type: RequiredOptionType });
type Optional = OptionalOptionType | (WrappedOptionType & { type: OptionalOptionType });
...
export type Config<O extends Options> = {
	[P in keyof O as O[P] extends Required | Group ? P : never]: O[P] extends Required
		? ValueOf<O[P]>
		: O[P] extends Group
			? ConfigGroup<O[P]>
			: never;
...
function configSchema<T extends Options>(options: T): v.Type<Config<T>> {
	function makeObjectType(options: Options | Group) {
		return v.object(
			Object.fromEntries(
				Object.entries(options).map(([name, value]): [string, OptionType | v.Type] => {
					// OptionType
...
type TerminalType = Parameters<Parameters<v.Type<unknown>['toTerminals']>[0]>[0];
...
export function parseOptions<T extends Options>(
	...
	return parseOptionsAdvanced(options, argv, envNamePrefix, false, false, processEnv, logger, exit)
		.config;
}
		...
		for (const [name, val] of Object.entries(options)) {
			...
			if (v.instanceOfAbstractType(val)) {
				...
				for (const [name, option] of Object.entries(val as Group)) {
					const wrapped = v.instanceOfAbstractType(option) ? { type: option } : option;
...
function showUsage(optionList: DescribedOptionDefinition[], logger: OptionalLogger = console) {
	const hide: string[] = [];
	let leftWidth = 35;
	let rightWidth = 70;
