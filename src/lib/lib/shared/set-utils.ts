...
export function equals<T>(a: ReadonlySetLike<T>, b: ReadonlySetLike<T>): boolean {
	if (a.size !== b.size) {
		return false;
	}
...
export function intersection<T>(a: ReadonlySetLike<T>, b: ReadonlySetLike<T>): Set<T> {
	const result = new Set<T>();
	if (a.size > b.size) {
 ...
 */
export function difference<T>(a: ReadonlySetLike<T>, b: ReadonlySetLike<T>): Set<T> {
	const result = new Set<T>();
	const iterator = a.keys();
	for (let { done, value } = iterator.next(); !done; { done, value } = iterator.next()) {
		if (!b.has(value)) {
...
export function symmetricDifferences<T>(
	...
	for (let { done, value } = iteratorA.next(); !done; { done, value } = iteratorA.next()) {
		onlyA.add(value);
	}

	const onlyB = new Set<T>();
	const iteratorB = b.keys();
	for (let { done, value } = iteratorB.next(); !done; { done, value } = iteratorB.next()) {
		if (a.has(value)) {
			onlyA.delete(value);
			onlyB.delete(value);
		} else {
