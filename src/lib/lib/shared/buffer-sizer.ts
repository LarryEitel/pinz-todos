...
export class BufferSizer {
	...
	recordMissable(now: number, missed: boolean, bufferNeededMs: number, lc: LogContext) {
		...
		const targetBufferNeededMs =
			this.#bufferNeededMsHistory[Math.floor((this.#bufferNeededMsHistory.length * 99.5) / 100)];
		...
		const missPercent =
			this.#missedCountSinceLastBufferAdjust / this.#missableCountSinceLastBufferAdjust;
		if (missPercent > 0.01) {
